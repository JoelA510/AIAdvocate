-- Allow service_role and other internal roles to bypass bill summary protection
...
BEGIN
  -- Get the current role from the session settings
  BEGIN
    current_role_setting := current_setting('role', true);
  EXCEPTION WHEN OTHERS THEN
    current_role_setting := NULL;
  END;

  -- IF service_role or postgres (internal): Allow everything (Skip checks)
  IF current_role_setting = 'service_role' OR current_user = 'postgres' OR current_user = 'supabase_admin' THEN
    RETURN NEW;
  END IF;

  -- Check if the current user is an admin via app_admins table
  -- We only do this if auth.uid() is available
  ...
  -- IF NOT ADMIN/INTERNAL: Apply original protection logic
