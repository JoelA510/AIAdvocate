...
>
> 1. **Batch A** — ✅ _device-verified on Android_ (sideloaded `internal-apk`, 2026-07-24): expo-updates re-enabled, edge-to-edge re-enabled, Sentry Android Gradle Plugin re-enabled, and the vestigial `ios.useFrameworks: "static"` dropped (it existed for `@react-native-firebase`, removed in PR #65; only `expo-notifications` remains, and it does not need static frameworks). **iOS is not device-verified** — the `useFrameworks` removal is an iOS-only linkage change and needs a TestFlight pass.
> 2. **Batch B** — ✅ _device-verified on Android_ (sideloaded `internal-apk`, 2026-07-24; R8 measured at **6.8 MB / 7.5%** smaller than the Batch A APK, which also proves R8 actually ran rather than silently no-opping): R8/Proguard + `shrinkResources` restored via `expo-build-properties` (android block only — the iOS `useFrameworks` entry stays dropped). Kept **solo** because it is the only one that can break the app subtly at _runtime_ while the build stays green (R8 strips/renames anything reached reflectively). Verification used the two-APK method: an identical build **without** R8 was tested first, so a failure would have been attributable to R8 with no further bisecting. Both passed. Needs the **full** smoke list above (line 59) — not a reduced one, and not just a launch check. In particular **hCaptcha and sign-in must be exercised**: hCaptcha is pure JS over `react-native-webview`, which is one of the libraries shipping no consumer ProGuard rules, making it the single most R8-exposed flow in the app. Also confirm images render (header banner, footer LNF icon) to cover resource shrinking, then: every tab, bill detail, rep lookup, share, TTS, push, toasts, language switch.
>
> **What actually protects the code from R8** (an earlier draft claimed "every native dependency ships its own consumer ProGuard rules" — that is **false**, and was verified false). Only `react-native`, `expo`, `expo-modules-core`, `expo-updates`, `react-native-reanimated` and `react-native-svg` declare `consumerProguardFiles`. `react-native-gesture-handler`, `react-native-webview`, `@react-native-async-storage/async-storage`, `@sentry/react-native`, `react-native-screens`, `react-native-safe-area-context`, and most `expo-*` modules ship **none**. What covers them is React Native core's blanket rules (`-keep class * implements com.facebook.react.bridge.NativeModule`, `@DoNotStrip`, `@ReactProp`), which reach any standard native module or view manager — but **not** classes reached reflectively by other means (Gson/Moshi models, `@Serializable`, `Serializable` POJOs persisted to SharedPreferences, WorkManager workers). No `extraProguardRules` are shipped because nothing in the current dependency set falls in that gap; that assumption must be re-checked whenever a native dependency is added. If the smoke test surfaces a strip crash, add a targeted `-keep` via `expo-build-properties.android.extraProguardRules` rather than disabling R8 wholesale.
>
> **Resources are a separate pass with a separate mitigation.** `shrinkResources` is an AAPT2 pass over `res/`, not ProGuard — a `-keep` rule cannot save a stripped resource. JS-`require()`d images and fonts land in `res/drawable-*`/`res/raw` and are resolved by name at runtime (`Resources.getIdentifier`), so they would be prime strip candidates; they survive because **Expo's CLI auto-generates `res/raw/keep.xml`** listing every Metro asset (`@expo/cli` `persistMetroAssets.js`). That covers the app's own images/fonts. It does **not** cover resources generated by other plugins (e.g. the `google-services` string resources), so if push or Firebase-backed behavior regresses under R8, suspect resource shrinking rather than ProGuard.
>
> **Sentry upload gates are ANDed** — `shouldSentryAutoUpload() = shouldSentryAutoUploadGeneral() && shouldSentryAutoUploadNative()` (`@sentry/react-native/sentry.gradle`). So `SENTRY_DISABLE_NATIVE_DEBUG_UPLOAD=true` suppresses the ProGuard **mapping** upload too, not just `.so` symbols. That is why `internal-apk` sets **neither** flag: a minified diagnostic build whose Java/Kotlin frames cannot be symbolicated is useless for the smoke test it exists to serve. `preview` still sets `SENTRY_DISABLE_AUTO_UPLOAD=true` and is also minified, so preview crashes will arrive obfuscated with no mapping — use `internal-apk`, not `preview`, to chase an R8 regression.
>
