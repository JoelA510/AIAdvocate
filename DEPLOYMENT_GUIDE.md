...
>
> **R8/Proguard is currently DISABLED** (expo-build-properties android block removed) and **edge-to-edge is currently DISABLED** (`android.edgeToEdgeEnabled` removed): the 1.7.0 builds hung on the native splash screen on-device. The R8-free rebuild still hung, which **exonerates R8 and the Sentry AGP as the cause** (they remain disabled only to keep the bisect single-variable; re-enable per the follow-ups below once the app launches). Edge-to-edge — the last untested native variable from the #61 batch — was removed next. A splash failsafe now force-hides after ~8s (`app/_layout.tsx`), which doubles as the diagnostic: **if a build hides the splash at ~8s**, JS runs and the hang is in startup work; **if the splash persists past ~10s**, the JS bundle never executes — then the next suspects are the expo-updates config from the same batch (`runtimeVersion: fingerprint` policy + `updates.url`) and, beyond that, the months of native-dependency drift since the last device-verified build (v1.0-era).
>
...
2. Profiles that must see production values (including `internal-apk`) pin `"environment": "production"`.
3. Defensive guards now exist and must stay: `src/lib/config.ts` and `src/lib/sentry.ts` treat any value containing `${` as missing, and `index.js` installs `src/boot/splash-failsafe.js` **before** `expo-router/entry` so a module-scope throw can no longer hide behind the splash screen.
4. After any change to env plumbing, verify the artifact, not the config: download the build, `unzip`, and check `assets/app.config` → `extra.publicEnv` contains real values (never `${`), and `grep -a '\${EXPO_PUBLIC' assets/index.android.bundle` finds nothing.
