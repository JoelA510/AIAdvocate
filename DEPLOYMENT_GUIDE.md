...
3. The build **fails on the EAS builder** if a required `EXPO_PUBLIC_*` value is missing **or contains `${`** (`app.config.ts` `collectPublicEnv()`), so a placeholder can no longer ship inside a binary.
