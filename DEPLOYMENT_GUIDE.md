...
>
> **R8/Proguard is currently DISABLED** (expo-build-properties android block removed): the first R8-instrumented build hung on the native splash screen on-device, and with no adb/pre-launch evidence the launch was unblocked by removing minification and the Sentry Android Gradle Plugin together. Re-enabling R8 is a tracked follow-up — do it as its own change, with a device on hand for the splash-screen check, re-adding `enableProguardInReleaseBuilds`/`enableShrinkResourcesInReleaseBuilds` first and `experimental_android.enableAndroidGradlePlugin` (Sentry mapping upload) second so the two variables stay separable.
