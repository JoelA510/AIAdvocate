...
>
> **R8/Proguard is currently DISABLED** (expo-build-properties android block removed): the first R8-instrumented build hung on the native splash screen on-device, and with no adb/pre-launch evidence the launch was unblocked by removing minification and the Sentry Android Gradle Plugin together. **If the un-minified build still hangs on the splash**, the remaining untested native variable from the same #61 commit is `android.edgeToEdgeEnabled` in `app.json` — disable that next; don't re-permute the R8/Sentry theories first.
>
