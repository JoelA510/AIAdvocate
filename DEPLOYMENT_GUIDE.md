...
>
> **Startup diagnostics**: the 1.7.0 builds hung on the native splash screen on-device, and none of the native suspects were to blame — the cause was the `eas.json` `${}` placeholder incident (see postmortem below). That bug was present in _every_ bisect build, which is why flipping native flags never produced a signal: each trial was "fatally broken config + one flag off", so the flag under test could never matter. R8, the Sentry AGP, edge-to-edge, and expo-updates are therefore **exonerated**, and the re-enable ladder below is a restoration of known-good settings, not a hunt.
>
...
>
> 1. ✅ **Batch A** (done): expo-updates re-enabled (`updates.enabled: true` with `fallbackToCacheTimeout: 0` so the app **never blocks startup on the network** — load embedded, fetch in background), edge-to-edge re-enabled, Sentry Android Gradle Plugin re-enabled, and the vestigial `ios.useFrameworks: "static"` dropped (it existed for `@react-native-firebase`, removed in PR #65; only `expo-notifications` remains and does not need it). Batched deliberately: none of these can cause a boot hang, and their failure modes are mutually distinguishable — edge-to-edge is _visual_, Sentry AGP fails the _build_ loudly, OTA problems show up as update targeting.
> 2. ⏳ **Batch B**: R8/Proguard + `shrinkResources`. Kept **solo** — it is the only one of the five that can break the app subtly at _runtime_ (R8 strips/renames anything reached reflectively) while the build stays green. Requires a full device smoke test, not just a launch check: every tab, bill detail, rep lookup, share, TTS, push, and language switch.
>
> **Edge-to-edge is not optional long-term**: Android 15 (API 35) enforces it for apps targeting it, and Play's API 36 deadline (~Aug 2026) with SDK 54 makes it mandatory. The `Toast topOffset={Math.max(40, insets.top + 16)}` in `_layout.tsx` exists specifically to keep toasts clear of the status bar under edge-to-edge.
>
> **Sentry AGP requires `SENTRY_AUTH_TOKEN`** to exist in the EAS environment used by the build or the build fails outright. It is present in the **production** environment (which `production` and `internal-apk` both pin); `development`/`preview` set `SENTRY_DISABLE_AUTO_UPLOAD=true` in `eas.json` instead, which the plugin's `shouldSentryAutoUpload()` gate respects. With R8 off there is no ProGuard mapping to upload and that task simply no-ops — the valuable part (native `.so` debug symbols for C++/Hermes frames) uploads regardless.
...
>
> **Migration note (vc7–vc13)**: these binaries shipped with `updates.enabled: false` (the bisect), so **OTA cannot reach them at all** — including **vc13, the build currently live in production**. The first OTA-capable binary is the next store build after Batch A. Until that build is live and adopted, every fix still requires a full store release.
