...
> 2. **Batch B** — _implemented, not device-verified_: R8/Proguard + `shrinkResources` restored via `expo-build-properties` (android block only — the iOS `useFrameworks` entry stays dropped). Kept **solo** because it is the only one that can break the app subtly at _runtime_ while the build stays green (R8 strips/renames anything reached reflectively). Needs the **full** smoke list above (line 59) — not a reduced one, and not just a launch check. In particular **hCaptcha and sign-in must be exercised**: hCaptcha is pure JS over `react-native-webview`, which is one of the libraries shipping no consumer ProGuard rules, making it the single most R8-exposed flow in the app. Also confirm images render (header banner, footer LNF icon) to cover resource shrinking, then: every tab, bill detail, rep lookup, share, TTS, push, toasts, language switch.
>
